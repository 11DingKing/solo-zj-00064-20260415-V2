...
public class Responses {
  ...
  public static void forbidden(HttpServletResponse response, String message) {
    ...
    try {
      ...
      response.setContentType("application/json");
      response.getWriter().write(JSON.stringify(
          new ApiError(message, FORBIDDEN)));
    } catch (Exception exception) {
      log.error("error writing forbidden response");
    }
  }
  ...
  public static void tooManyRequests(HttpServletResponse response, String message) {
    if (response.getStatus() != 200) {
      return;
    }
    ...
    try {
      ...
      response.getWriter().write(JSON.stringify(
          new ApiError(message, TOO_MANY_REQUESTS)));
    } catch (Exception exception) {
      log.error("error writing too many requests response");
    }
  }
